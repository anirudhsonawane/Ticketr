...
export const getUserWaitingStatus = query({
  args: {
    eventId: v.id("events"),
    userId: v.string(),
  },
...
export const syncWaitingListStatus = mutation({
  args: {
    eventId: v.id("events"),
    userId: v.string(),
  },
  handler: async (ctx, { eventId, userId }) => {
    // Check if user has valid tickets for this event
    ...
    // Get all waiting list entries for this user and event
    const waitingEntries = await ctx.db
      .query("waitingList")
      .withIndex("by_user_event", (q) => q.eq("userId", userId).eq("eventId", eventId))
      .collect();

    // If user has tickets, mark all waiting list entries as purchased
    if (userTickets.length > 0) {
      for (const entry of waitingEntries) {
        if (entry.status !== WAITING_LIST_STATUS.PURCHASED) {
          await ctx.db.patch(entry._id, {
            status: WAITING_LIST_STATUS.PURCHASED,
          });
        }
      }
      return { status: "purchased", updated: waitingEntries.length };
    }

    // If no tickets, check for expired offers
    const now = Date.now();
    for (const entry of waitingEntries) {
      if (entry.status === WAITING_LIST_STATUS.OFFERED && 
          (entry.offerExpiresAt ?? 0) <= now) {
        await ctx.db.patch(entry._id, {
          status: WAITING_LIST_STATUS.EXPIRED,
        });
      }
    ...
    return { status: "synced", updated: waitingEntries.length };
  },
});